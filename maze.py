...
class Maze:
    ...
    def load_maze(self, filename):
        ...
        for y, line in enumerate(self.maze_lines):
            for x, char in enumerate(line.strip()):
                if char == '#':
                    self.walls.append(self.pygame.Rect(x*self.WALL_SIZE, y*self.WALL_SIZE, self.WALL_SIZE, self.WALL_SIZE))

    def draw(self, screen):
        for wall in self.walls:
            self.pygame.draw.rect(screen, self.WALL_COLOR, wall)
        exit_x, exit_y = self.exit_location
        self.pygame.draw.rect(screen, self.EXIT_COLOR, (exit_x * self.WALL_SIZE, exit_y * self.WALL_SIZE, self.WALL_SIZE, self.WALL_SIZE))
